...
class RLAgent:
    ...
    def select_action(self, state: list[float]) -> tuple[int, list[float]]:
        """Select action using epsilon-greedy policy.

        Args:
            state: Feature vector of length state_dim.

        Returns:
            Tuple of (action_index, q_values_list).
        """
        if not TORCH_AVAILABLE or self._policy_net is None:
            action = random.randint(0, self._action_dim - 1)
            return action, [0.0] * self._action_dim
    ...
    def train_step(self) -> float | None:
        ...
        actions = torch.LongTensor(batch["actions"]).to(self._device)
    ...
    def end_episode(self, episode_reward: float) -> None:
        """Called at end of each episode to update epsilon and counters."""
        self._episode += 1
