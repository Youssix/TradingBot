"""Tests for RL agent, environment, and replay buffer (continuous actions)."""
...
class TestReplayBuffer:
    def test_push_and_len(self):
        ...
        for i in range(10):
            buf.push(Transition(
                state=[float(i)] * 5,
                action=float(i % 3) - 1.0,  # continuous float
                reward=float(i),
    ...
    def test_capacity_overflow(self):
        ...
        for i in range(10):
            buf.push(Transition(
                state=[0.0], action=0.0, reward=0.0, next_state=[0.0],
            ))
        assert len(buf) == 5
    ...
    def test_sample_size(self):
        ...
        for i in range(20):
            buf.push(Transition(
                state=[float(i)], action=0.5, reward=1.0, next_state=[float(i)],
            ))
        batch = buf.sample(5)
        assert len(batch) == 5
    ...
    def test_sample_arrays(self):
        ...
        assert arrays["actions"].dtype == np.float32  # continuous
    ...
    def test_clear(self):
        ...
        buf.push(Transition(state=[1.0], action=0.0, reward=0.0, next_state=[1.0]))
        buf.clear()
        assert len(buf) == 0

    def test_to_list(self):
        buf = ReplayBuffer(capacity=10)
        buf.push(Transition(state=[1.0], action=0.8, reward=0.5, next_state=[2.0], done=True))
        data = buf.to_list()
        assert len(data) == 1
        assert data[0]["action"] == 0.8
...
class TestTradingEnvironment:
    ...
    def test_buy_and_close(self):
        ...
        # Open BUY with positive action > threshold
        reward, result = env.step(0.8, 1950.0, [0.0] * 23)
        assert env.in_position
        assert result is None

        # Hold for a few bars (same direction)
        for _ in range(5):
            env.step(0.8, 1951.0, [0.0] * 23)

        # Close with opposing negative action
        reward, result = env.step(-0.8, 1955.0, [0.0] * 23)
    ...
    def test_sell_and_close(self):
        env = TradingEnvironment()
        env.step(-0.8, 1960.0, [0.0] * 23)
        assert env.in_position

        for _ in range(5):
            env.step(-0.8, 1958.0, [0.0] * 23)
    ...
    def test_losing_trade(self):
        env = TradingEnvironment()
        env.step(0.8, 1950.0, [0.0] * 23)
        for _ in range(5):
            env.step(0.8, 1948.0, [0.0] * 23)
        reward, result = env.step(-0.8, 1940.0, [0.0] * 23)
        assert result is not None
        assert result.pnl < 0
        assert result.reward < 0
    ...
    def test_hold_penalty(self):
        ...
        # Small action magnitude = hold (below open_threshold)
        reward, _ = env.step(0.1, 1950.0, [0.0] * 23)
        assert reward == -0.05
    ...
    def test_max_hold_force_close(self):
        ...
        env.step(0.8, 1950.0, [0.0] * 23)
        result = None
        # Low magnitude action (hold) triggers force-close after max_hold_bars
        for i in range(10):
            _, result = env.step(0.05, 1950.0 + i * 0.1, [0.0] * 23)
            if result:
                break
    ...
    def test_threshold_behavior(self):
        """Actions below open_threshold should not open positions."""
        env = TradingEnvironment(open_threshold=0.3)
        # Action = 0.2 < 0.3 threshold, should not open
        reward, result = env.step(0.2, 1950.0, [0.0] * 23)
        assert not env.in_position
        # Action = 0.5 > 0.3 threshold, should open
        reward, result = env.step(0.5, 1950.0, [0.0] * 23)
        assert env.in_position
    ...
    def test_action_clipping(self):
        ...
        env.step(2.0, 1950.0, [0.0] * 23)
        assert env.in_position


class TestRLAgent:
    def test_select_action_shape(self):
        """DQN agent should return (float, dict) with action in [-1, 1]."""
    ...
    def test_store_transition(self):
        agent = RLAgent(state_dim=5, buffer_capacity=100)
        t = Transition(state=[0.0] * 5, action=0.5, reward=0.5, next_state=[0.0] * 5)
        agent.store_transition(t)
        assert agent.buffer_size == 1

    @pytest.mark.skipif(not TORCH_AVAILABLE, reason="PyTorch not installed")
    def test_train_step_insufficient_data(self):
        agent = RLAgent(state_dim=5, batch_size=32)
        for i in range(5):
            agent.store_transition(Transition(
                state=[float(i)] * 5, action=0.5, reward=0.1, next_state=[0.0] * 5,
            ))
    ...
    def test_train_step_runs(self):
        agent = RLAgent(state_dim=5, batch_size=16)
        for i in range(50):
            agent.store_transition(Transition(
                state=[float(i % 10)] * 5,
                action=float(i % 3) * 0.5 - 0.5,
    ...
    def test_base_agent_interface(self):
        """RLAgent should satisfy BaseAgent interface."""
